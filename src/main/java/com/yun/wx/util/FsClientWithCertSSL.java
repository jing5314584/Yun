	...
	private static CloseableHttpClient createClient(RequestConfig config) {
		...
		try {
			...
			//FileInputStream instream = new FileInputStream(new File(CERT));
			 String certificatePath =  FsClientWithCertSSL.class.getClassLoader().getResource("apiclient_cert.p12").getPath();
			InputStream instream = FsClientWithCertSSL.class.getResourceAsStream(certificatePath);
			try {
			// 指定PKCS12的密码(商户ID)
